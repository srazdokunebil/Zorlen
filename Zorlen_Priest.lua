...
function isShadowVulnerability(unit, dispelable)
	return Zorlen_checkDebuff("Shadow_BlackPlague", unit, dispelable)
end

function isShadowVulnerabilityFull(unit)
	if not (Zorlen_GetDebuffStack("Shadow_BlackPlague", unit) == 5) then
		return true
	else
		return false
	end
...
function isShadowVulnerabilityStacks(unit, stacks)
    s = Zorlen_GetDebuffStack("Shadow_BlackPlague", unit)
	if s <= stacks then
	    --print('mob has ' .. s .. ' stacks of shadow vulnerability');
		return true
	else
		return false
	end
...
function isInnerFocusActive()
	return Zorlen_checkBuff("Spell_Frost_WindWalkOn")
end
...
function castShadowWordPain(SpellRank, force)
...
--Casts Inner Focus if cooldown is up
function castInnerFocus(SpellRank)
	if Zorlen_isMoving() then
		return false
	end
